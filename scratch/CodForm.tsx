...
export function CodForm({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const wilayaName = selectedWilayaObj ? ${selectedWilayaObj.code} -  : form.wilaya;
      const calculatedDeliveryFee = form.shippingMethod === 'home' ? selectedWilayaObj?.home : selectedWilayaObj?.stop;
      
      const eventId = crypto.randomUUID ? crypto.randomUUID() : evt__;

      const finalAddress = form.note ? \\ | ملاحظة: \\ : form.address;
      ...
      <form onSubmit={handleSubmit} className="space-y-10" onFocus={handleFormInteraction} onClick={handleFormInteraction}>
              ...
              {offers.map((offer: any) => {
                ...
                return (
                  <button
                    ...
                    className={\elative flex items-center justify-between p-5 rounded-2xl border-2 transition-all text-right \\}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className={\ont-bold text-lg \\}>{offer.title}</p>
                    ...
                    {variants.map((v: any) => {
                      ...
                      return (
                        <button
                          ...
                          className={\ounded-xl min-w-[4rem] border-2 px-5 py-3 text-base uppercase tracking-wider transition-all font-bold \\}
                        >
                           {v.node.title}
                           {!isAvailable && !isSelected && (
                              <div className="absolute inset-0 bg-slate-200/50 flex items-center justify-center rotate-45 transform origin-center" style={{height: '1px', width: '200%', top: '50%', left: '-50%'}}></div>
                           )}
                        </button>
                      );
                    })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
            ...
            <div>
              ...
              <input 
                ...
                className={\\ text-right placeholder:text-right\} placeholder="05XX XX XX XX"
              />
            </div>
        ...
        <section id="shipping-method">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
            <label className={\elative flex items-center p-5 border-2 rounded-2xl cursor-pointer transition-all \\}>
             ...
             <div className="mt-5 animate-in fade-in slide-in-from-top-4 duration-300">
               ...
               <div className="relative">
                  <input 
                    ...
                    className={\\ pr-12\} placeholder="اسم الحي، الشارع، أو أقرب مَعلَم (اختياري)"
                  />
          ...
          <div className="mt-5">
             ...
             <textarea 
               ...
               className={\\ resize-none\} placeholder="أي ملاحظات حول الطلب أو التوصيل..."
             />
          </div>
        </section>
